...
try {
    ...
    if (error.message == "timeoutError"){error_message = this.homey.__("ECU_timeout")}
    await this.homey.flow.getTriggerCard('ECU_error').trigger({'error_message':error.message});
...
async pollLoop() {
  ...
  try {
    ...
    console.log(`⏸️ Polling is running.`);
